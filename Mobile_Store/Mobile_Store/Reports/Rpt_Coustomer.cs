...
{
    public partial class Rpt_Coustomer : Form
    {
        PersianCalendar pc = new PersianCalendar();
        List<SanadMoshtari> lst_moshtari = new List<SanadMoshtari>();
        ...
        public void load_form()
        {
            try
            {
                ...
                foreach (SanadMoshtari m in lst_moshtari)
                {
                    ...
                    t.SubItems.Add(m.Shomareh_sanad.ToString());
                    t.SubItems.Add(m.Name_Moshtari);
                    t.SubItems.Add(m.Bedehkar.ToString());
                    jamhe_bedehkar += m.Bedehkar;
                    t.SubItems.Add(m.Bestankar.ToString());
                    jamhe_bestankar += m.Bestankar;
                    t.SubItems.Add(m.Date);
                    t.SubItems.Add(m.Tozih);
        ...
        private void Tarikh(SanadMoshtari item)
        {
        ...
        private void Search()
        {
            var q = context.SanadMoshtaris.Where(f => f.Name_Moshtari.EndsWith(cmb_moshtari.Text));
            foreach (var item in q)
            {
                Tarikh(item);
                if (tarikh_start <= tarikh_sanad && tarikh_sanad <= tarikh_end) lst_moshtari.Add(item);
            }
        }
        private void Rpt_Coustomer_Load(object sender, EventArgs e)
        {
            try
            {
                Date();
        ...
        private void btn_search_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var q = context.SanadMoshtaris.Select(s => s);
                    foreach (var item in q)
                    {
                        Tarikh(item);
                        if (tarikh_start <= tarikh_sanad && tarikh_sanad <= tarikh_end) lst_moshtari.Add(item);
                ...
                lab_jamhe_bedehi.Text = jamhe_bedehkar.ToString();
            }
